...
class Search {
      ...
      this.resultsDiv.innerHTML = `
            <div class="row">
                ...
                ${results.programs.length ? "</ul>" : ""}
					...
					${results.professors.length ? '<ul class="link-list min-list">' : `<p>No professors matches that search. <a href="${universityData.root_url}/professors">View all professors</a></p>`}
  ...
  openOverlay(event) {
    event.preventDefault();
    event.stopPropagation();
    this.searchOverlay.classList.add("search-overlay--active"); // adds css property overflow:hidden

    document.body.classList.add("body-no-scroll"); // clears search field on overlay open

    this.searchField.innerHTML = ""; // waits for overlay to fade in before focusing cursur in input field.
